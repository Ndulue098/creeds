...
export default function AnnouncementList({announcements}) {
  ...
  return (
    <>
      ...
      <div
        className="flex h-full flex-col gap-3 items-center overflow-y-scroll lg:col-span-6 lg:grid-cols-none"
        // onClick={top}
      >
        {announcements.map((announcement, i) => (
        <AnnouncementCard val={announcement} topAnnounce={topAnnounce} top={() => setTop(announcement)} key={i}/>
        ))}
      </div>
    </>
  );
